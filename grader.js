...
var URL_DEFAULT = 'http://shrouded-cove-4178.herokuapp.com/';
...
/*var cheerioURLFile = function(urladdress){
    rest.get(urladdress).on('complete', function(result){
	if(result instanceof Error){
	    console.error('Error: ' + result.message);
	}
	else{
	    console.error("Opened %s", urladdress);
	    return cheerio.load(result);
	}
    });
};*/

var checkURLFile = function(urladdress, checksfile){
    rest.get(urladdress).on('complete', function(result){
	if(result instanceof Error){
	    console.error('Error: ' + result.message);
	}
